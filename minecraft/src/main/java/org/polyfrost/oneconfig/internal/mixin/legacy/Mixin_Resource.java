...
@Mixin(value = Resource.class, remap = false)
public interface Mixin_Resource {
	@WrapOperation(
		method = "lambda$supplier$0",
		at = @At(
			value = "INVOKE",
			target = "Ljava/nio/file/Files;newInputStream(Ljava/nio/file/Path;[Ljava/nio/file/OpenOption;)Ljava/io/InputStream;"
		)
	)
	private static InputStream oneconfig$deferInputStream(Path path, OpenOption[] options, Operation<InputStream> original) throws IOException {
