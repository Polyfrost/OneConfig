...
import org.polyfrost.oneconfig.utils.commands.arguments.EntityPlayerArgumentParser;
import org.polyfrost.oneconfig.utils.commands.arguments.PlayerArgumentParser;
    ...
    public void createCommand(CommandManager.OCCommand root) {
        ClientCommandHandler.instance.registerCommand(new CommandBase() {
                ...
                //#endif
            {
                ...
                try {
                    if (command != null) {
                        ...
                        opts.addAll(CommandManager.INSTANCE.parsers.get(currentParam.getType()).complete(args[args.length - 1], currentParam));
                    }
            ...
            private String[] doCommand(@NotNull String[] args) {
                ...
                return new String[]{root.getMetadata().chatColor() + CommandManager.NOT_FOUND_TEXT.replace("@ROOT_COMMAND@", root.getMetadata().value())};
            }
            ...
            private CommandManager.Pair<String[], CommandManager.InternalCommand> getCommand(String[] args) {
                ...
                String argsIn = String.join(CommandManager.DELIMITER, args).toLowerCase();
                for (int i = args.length - 1; i >= 0; i--) {
                    ...
                    if (command != null) {
                        String primaryPath = command.getPrimaryPath()
                                .replace(CommandManager.DELIMITER + CommandManager.MAIN_METHOD_NAME, "")
                                .replace(CommandManager.MAIN_METHOD_NAME, "");
                        int skipArgs = 0;
                        if (!primaryPath.isEmpty()) skipArgs++;
                        for (char c : primaryPath.toCharArray()) {
                            if (c == CommandManager.DELIMITER.toCharArray()[0]) skipArgs++;
                        }
                    ...
                    argsIn = StringUtils.substringToLastIndexOf(argsIn, CommandManager.DELIMITER);
                }
            ...
            private CommandManager.InternalCommand get(CommandManager.OCCommand command, String in) {
                for (String[] ss : command.commandsMap.values()) {
                    for (String s : ss) {
                        if (s.equalsIgnoreCase(in) || s.equalsIgnoreCase(in + CommandManager.DELIMITER + CommandManager.MAIN_METHOD_NAME)) {
                ...
                String[] argsIn = in.toLowerCase().split(CommandManager.DELIMITER);
            ...
            private Collection<String> getApplicableOptsFor(String[] args) {
                ...
                final String current = String.join(CommandManager.DELIMITER, args);
                root.commandsMap.values().forEach(paths -> {
                    for (String p : paths) {
                        if (p.endsWith(CommandManager.MAIN_METHOD_NAME)) continue;
                        if (!p.startsWith(current)) continue;
                        final String[] split = p.split(CommandManager.DELIMITER);
    ...
    private static CommandManager.Pair<String, CommandManager.InternalCommand> getFallback(CommandManager.OCCommand command, String in) {
        ...
        if (in.isEmpty()) {
            ...
            CommandManager.InternalCommand cmd = command.commandsMap.entrySet().stream()
                    .filter(e -> Arrays.asList(e.getValue()).contains(CommandManager.MAIN_METHOD_NAME))
                    .map(Map.Entry::getKey)
            ...
            return new CommandManager.Pair<>(CommandManager.MAIN_METHOD_NAME, cmd);
        }

        String[] splitData = in.split(CommandManager.DELIMITER);
        for (int i = splitData.length; i >= 0; i--) {
            String[] split = Arrays.copyOfRange(splitData, 0, i);
            String path = String.join(CommandManager.DELIMITER, split).trim();
            ...
            for (Map.Entry<CommandManager.InternalCommand, String[]> entry : command.commandsMap.entrySet()) {
                ...
                for (String cmdPath : acceptedPaths) {
                    ...
                    boolean matchesMain = cmdPath.equals(path + (path.isEmpty() ? "" : CommandManager.DELIMITER) + CommandManager.MAIN_METHOD_NAME.toLowerCase(Locale.ROOT));
