package cc.polyfrost.oneconfig.api.commands;

import cc.polyfrost.oneconfig.api.commands.annotations.Main;
import cc.polyfrost.oneconfig.api.commands.arguments.*;
import cc.polyfrost.oneconfig.api.events.commands.arguments.*;
import cc.polyfrost.oneconfig.api.commands.annotations.Command;
import cc.polyfrost.oneconfig.api.commands.annotations.Greedy;
        ...
        while (processed < args.length) {
            ...
            try {
                ...
                try {
                    ...
                    return Collections.singletonList(new CustomError("A " + e.getClass().getSimpleName() + " has occurred while try to parse " + param.getType().getSimpleName() + "! Please report this to the mod author."));
                }
            } catch (Exception e) {
                return Collections.singletonList(new CustomError("A " + e.getClass().getSimpleName() + " has occurred while try to parse " + param.getType().getSimpleName() + "! Please report this to the mod author."));
            }
