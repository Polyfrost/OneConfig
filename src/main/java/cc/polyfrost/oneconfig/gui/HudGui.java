...
public class HudGui extends UScreen implements GuiPause {
    private ArrayList<Hud> editingHuds = new ArrayList<>(); // allow selection of multiple HUDS
    ...
    public void onDrawScreen(@NotNull UMatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        if (draggingHud != null) {
            for (Hud hud : editingHuds) {
                ...
                position.setPosition(position.getX() + mouseX - lastX, position.getY() + mouseY - lastY);
            }
            this.lastX = mouseX;
            this.lastY = mouseY;
        }
