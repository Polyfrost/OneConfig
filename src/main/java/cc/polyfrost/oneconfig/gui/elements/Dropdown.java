...
public class Dropdown extends BasicButton {
    ...
    public void draw(long vg, float x, float y, InputHandler inputHandler) {
        if (optsWidth == -1) {              // lateinit
            for (String s : opts.keySet()) {
                optsWidth = Math.max(optsWidth, RenderManager.getTextWidth(vg, s, 12, Fonts.REGULAR));
            }
        ...
        if (anim != null) {
            ...
            RenderManager.drawRoundedRect(vg, x, y + height, optsWidth, val, colorPalette == ColorPalette.TERTIARY ? ColorPalette.SECONDARY.getNormalColor() : currentColor, 8);
            if (anim.isFinished() && !anim.isReversed() && toggled) {
                ...
                for (Map.Entry<String, ColorAnimation> entry : opts.entrySet()) {
                    ...
                    RenderManager.drawRoundedRect(vg, x, i, optsWidth, 24, anim.getColor(hovered, clicked), 8);
                    RenderManager.drawText(vg, s, x + 8, i + 12, Colors.WHITE_90, 12, Fonts.REGULAR);
                    if (clicked) {
    ...
    public void drawIcon(long vg, SVG icon, float x, float y, float width, float height, int color) {
        ...
        RenderManager.translate(vg, xx, yy);
        RenderManager.rotate(vg, rotation);
        RenderManager.drawSvg(vg, icon, -(width / 2), -(height / 2), width, height, color);
        RenderManager.rotate(vg, -rotation);
        RenderManager.translate(vg, -xx, -yy);
    }
