...
package cc.polyfrost.oneconfig.config.v1.processor;

import cc.polyfrost.oneconfig.config.v1.OneConfig;
import cc.polyfrost.oneconfig.config.v1.options.OptionHolder;
import cc.polyfrost.oneconfig.config.v1.processor.collector.AnnotationCollector;
import cc.polyfrost.oneconfig.config.v1.processor.collector.Collector;
import cc.polyfrost.oneconfig.config.v1.processor.serializers.GsonSerializer;
import cc.polyfrost.oneconfig.config.v1.processor.serializers.Serializer;
...
public class OptionProcessor {
    ...
    public void serialize(OneConfig config, List<OptionHolder> options) {
        for (Property property : config.getProperties().getProperties()) {
    ...
    /**
     * Deserializes the given {@link OneConfig} from {@link OneConfig#getFile()} to the given {@link OptionHolder}s.
     * This is done by iterating through all {@link Serializer} objects in {@link #serializers}.
