...
package cc.polyfrost.oneconfig.config.v1.processor.collector;

import cc.polyfrost.oneconfig.config.v1.OneConfig;
import cc.polyfrost.oneconfig.config.v1.options.OptionHolder;
import cc.polyfrost.oneconfig.config.v1.options.OptionManager;
...
import cc.polyfrost.oneconfig.config.v1.options.type.annotations.Type;
import cc.polyfrost.oneconfig.config.v1.utils.ConfigUtils;
...
public class AnnotationCollector implements Collector {
    ...
    public List<OptionHolder> collect(OneConfig config) {
        ...
        for (Field field : fields) {
            Type annotation = ConfigUtils.findAnnotation(field, Type.class);
            if (annotation != null) {
        ...
        for (Method method : methods) {
            Type annotation = ConfigUtils.findAnnotation(method, Type.class);
            if (annotation != null) {
                OptionType optionType = OptionManager.INSTANCE.getOption(annotation.value());
                if (optionType == null) {
                    throw new RuntimeException("Option " + annotation.value() + " does not exist!");
