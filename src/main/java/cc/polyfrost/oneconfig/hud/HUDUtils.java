...
public class HUDUtils {
    public static void addHudOptions(OptionPage page, Field field, Object instance, Config config) {
        ...
        try {
            ...
            options.add(new ConfigSlider(fields.get("scale"), hud, "Scale", "The scale of the HUD", category, subcategory, 0.3f, 10f, 1, 0, false));
            ...
            if (hud instanceof BasicHud) {
                ...
                options.add(new ConfigSlider(fields.get("cornerRadius"), hud, "Corner radius:", "The corner radius of the background.", category, subcategory, 0, 10, 1, 0, false));
                options.get(options.size() - 1).addDependency("Rounded", () -> ((BasicHud) hud).rounded);
                options.add(new ConfigSlider(fields.get("borderSize"), hud, "Border thickness:", "The thickness of the outline.", category, subcategory, 0, 10, 1, 0, false));
                options.get(options.size() - 1).addDependency("Border", () -> ((BasicHud) hud).border);
                ...
                options.add(new ConfigSlider(paddingX, hud, "X-Padding", "The horizontal padding of the HUD.", category, subcategory, 0, 10, 1, 0, false));
                options.add(new ConfigSlider(paddingY, hud, "Y-Padding", "The vertical padding of the HUD.", category, subcategory, 0, 10, 1, 0, false));
