...
public class HUDUtils {
    public static void addHudOptions(OptionPage page, Field field, Object instance, Config config) {
        ...
        try {
            ...
            boolean isPatcher = Platform.getLoaderPlatform().isModLoaded("patcher");
            ...
            options.add(new ConfigSwitch(fields.get("locked"), hud, "Locked", "If the position is locked", category, subcategory, isPatcher ? 1 : 2));
            if (isPatcher) options.add(new ConfigSwitch(fields.get("ignoreCaching"), hud, "Ignore Hud Caching", "Ignore patcher's Hud caching", category, subcategory, 1));
